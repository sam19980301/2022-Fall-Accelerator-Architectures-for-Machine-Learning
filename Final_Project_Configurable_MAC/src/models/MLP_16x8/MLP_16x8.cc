...
#include "models/MLP_16x8/MLP_16x8.h"

#include <stdio.h>

#include "menu.h"
#include "models/MLP_16x8/model_MLP_16x8.h"
#include "models/MLP_16x8/image_data.h"
#include "tflite.h"
...
static void MLP_16x8_init(void) {
  tflite_load_model(model_MLP_16x8, model_MLP_16x8_len);
}

// Run classification, after input has been loaded
static int32_t MLP_16x8_classify() {
  printf("Running MLP_16x8\n");
  tflite_classify();
...
static void do_classify_zeros() {
  ...
  int32_t result = MLP_16x8_classify();
  printf("  result is %ld\n", result);
}

static void do_classify() {
  tflite_set_input(MLP_16x8_data);
  int32_t result = MLP_16x8_classify();
  printf("  result is %ld\n", result);
}
...
static void do_golden_tests() {
  ...
  tflite_set_input_zeros();
  actual[0] = MLP_16x8_classify();
  
  tflite_set_input(MLP_16x8_data);
  actual[1] = MLP_16x8_classify();
