...
#include "models/MLP_8x8/MLP_8x8.h"

#include <stdio.h>

#include "menu.h"
#include "models/MLP_8x8/model_MLP_8x8.h"
#include "models/MLP_8x8/image_data.h"
#include "tflite.h"
...
static void MLP_8x8_init(void) {
  tflite_load_model(model_MLP_8x8, model_MLP_8x8_len);
}

// Run classification, after input has been loaded
static int32_t MLP_8x8_classify() {
  printf("Running MLP_8x8\n");
  tflite_classify();
...
static void do_classify_zeros() {
  ...
  int32_t result = MLP_8x8_classify();
  printf("  result is %ld\n", result);
}

static void do_classify() {
  tflite_set_input(MLP_8x8_data);
  int32_t result = MLP_8x8_classify();
  printf("  result is %ld\n", result);
}

#define NUM_GOLDEN 2
static int32_t golden_results[NUM_GOLDEN] = {68, -12};

static void do_golden_tests() {
  int32_t actual[NUM_GOLDEN];

  tflite_set_input_zeros();
  actual[0] = MLP_8x8_classify();
  
  tflite_set_input(MLP_8x8_data);
  actual[1] = MLP_8x8_classify();
