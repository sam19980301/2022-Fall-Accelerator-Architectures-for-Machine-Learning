...
#include "models/resnet/resnet.h"

#include <stdio.h>

#include "menu.h"
#include "models/resnet/model_resnet.h"
#include "models/resnet/image_data.h"
#include "tflite.h"
...
static void resnet_init(void) {
  tflite_load_model(model_resnet, model_resnet_len);
}

// Run classification, after input has been loaded
static int32_t resnet_classify() {
  printf("Running resnet\n");
  tflite_classify();

  // Process the inference results.
  int8_t* output = tflite_get_output();
  return output[9] - output[0];
}

static void do_classify_zeros() {
  tflite_set_input_zeros();
  int32_t result = resnet_classify();
  printf("  result is %ld\n", result);
}

static void do_classify() {
  tflite_set_input(resnet_data);
  int32_t result = resnet_classify();
  printf("  result is %ld\n", result);
}

#define NUM_GOLDEN 2
static int32_t golden_results[NUM_GOLDEN] = {-59, 65};

static void do_golden_tests() {
  int32_t actual[NUM_GOLDEN];

  tflite_set_input_zeros();
  actual[0] = resnet_classify();
  
  tflite_set_input(resnet_data);
  actual[1] = resnet_classify();
